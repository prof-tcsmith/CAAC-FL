"""
Level 3: Krum with Byzantine Attacks

Runs Krum aggregation with configurable Byzantine attacks.
Tests distance-based Byzantine-robust aggregation.
"""
...
# Import Krum from Level 2
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../level2_heterogeneous')))
from krum_strategy import Krum


def parse_args():
    """Parse command line arguments"""
    parser = argparse.ArgumentParser(description='Level 3: Krum with Byzantine Attacks')
    parser.add_argument(
        '--attack',
        type=str,
        default='none',
...
def main():
    ...
    print("=" * 70)
    print("Level 3: Krum with Byzantine Attacks")
    print("=" * 70)
    print(f"Configuration:")
    print(f"  Aggregation Method: Krum")
    ...
    print(f"  Krum Byzantine Tolerance (f): {NUM_BYZANTINE}")
    ...
    logger = MetricsLogger(
        log_dir='./results',
        experiment_name=f'level3_krum{attack_suffix}'
    )
    ...
    # Configure strategy (Krum)
    strategy = Krum(
        num_byzantine=NUM_BYZANTINE,  # Set Byzantine tolerance
    ...
    # Start simulation
    print(f"\nStarting Krum simulation ({NUM_ROUNDS} rounds)...")
