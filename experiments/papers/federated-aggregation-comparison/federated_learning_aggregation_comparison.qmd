...
Federated Learning (FL) has emerged as a paradigm-shifting approach to distributed machine learning, enabling collaborative model training across decentralized data sources while preserving data privacy. A critical component of FL systems is the aggregation strategy used to combine local model updates from participating clients. This paper presents a **multi-dimensional empirical study** comparing fundamental aggregation strategies—Federated Averaging (FedAvg), Federated Mean (FedMean), and Federated Median (FedMedian)—across three experimental axes: **(1) data distribution heterogeneity** (IID-Equal vs. IID-Unequal client dataset sizes), **(2) client scalability** (10, 25, 50 clients), and **(3) non-IID data challenges** (Dirichlet distribution with varying concentration parameters). Through systematic experimentation on CIFAR-10 with 18 distinct experimental configurations, we provide comprehensive insights into aggregation strategy performance under realistic federated settings.

Our key findings demonstrate: **(1)** FedAvg's dataset-size weighting provides a measurable advantage (+2.01%) over FedMean when client datasets are heterogeneous in size, validating the importance of proportional weighting; **(2)** accuracy systematically degrades as client count increases (78.8% → 75.6% → 73.0% for FedAvg), suggesting convergence challenges at scale; and **(3)** FedMedian maintains competitive performance while offering theoretical robustness guarantees. These results establish empirical baselines for aggregation strategy selection and inform the design of next-generation Byzantine-robust federated learning systems.
...
**Figure 3**: Heatmap of final test accuracy across all experimental configurations. Rows represent partition types and client counts; columns represent aggregation methods.

The heatmap provides a holistic view of experimental results:

**Diagonal Pattern**: Accuracy generally decreases from top-left (best: IID-Equal, 10 clients, FedAvg) to bottom-right (worst: IID-Unequal, 50 clients, FedMedian).

**Column Comparison**: FedAvg and FedMean columns show similar patterns with IID-Equal, but diverge with IID-Unequal (validating H1). FedMedian column is consistently cooler (lower accuracy).

**Row Comparison**: Rows representing 10-client experiments are warmer (higher accuracy) than 50-client experiments (validating H2).
